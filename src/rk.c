    ...
    void *par)
{
   ...
   f(k[0], this_y, this_x, false, par);
    ...
    number_multiplication_array(jump, step_x/2, k[1]);
    array_addition(jump_y, this_y, jump);

    /// k_2 = f(x_n + h/2, y_n + k_2 * h/2)
    f(k[2], jump_y, this_x + step_x/2, true, par);
    number_multiplication_array(jump, step_x, k[2]);
    array_addition(jump_y, this_y, jump);
    ...
    /// 2*k_1
    number_multiplication_array(k[1], 2, k[1]);
    /// 2*k_2
    number_multiplication_array(k[2], 2, k[2]);

    /// k_3 = k_0 + 2*k_1 + 2*k_2 + k_3
    for (size_t i = 0; i < 3; i++)
    {
        array_addition(k[3], k[i], k[3]);
    }
    
    /// (k_0 + 2*k_1 + 2*k_2 + k_3)*h/6
    number_multiplication_array(k[3], step_x / 6, k[3]);
    /// y_{n+1} = y_n + (k_0 + 2*k_1 + 2*k_2 + k_3)*h/6
    array_addition(next_y, this_y, k[3]);
